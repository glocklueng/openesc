...
void	Calc_next_commutation_detectZero(Commutate_EC_Def* comm_EC, uint32_t period)
{
	...
	temp = (period * (uint32_t)pre_commutation) >> 16; 
...
uint32_t	Calc_PZC(Commutate_EC_Def* comm_EC, TIM_TypeDef* TIMx)
{
	...
	if (comm_EC->Flag_enable_feedback == 1)	// update time event commutate (FB by Emf)
	{
...
void Detect_pos_edge(Commutate_EC_Def* comm_EC, uint16_t Uemf, uint16_t MidleUin, TIM_TypeDef* TIMx)
{
  	if (comm_EC->Flag_enable_ZC == 1)
	{
		...
		{
                        LED_STATUS_OFF();
			comm_EC->Flag_missed_ZC = 1;	// zero crossing is detected!
		  	comm_EC->P_ZC = Calc_PZC(comm_EC, TIMx);
			comm_EC->Flag_enable_ZC = 0;	// ban detecting zero crossing!
		}
		else
...
uint16_t	DetectZeroCross(uint16_t Uback, uint16_t Uin, Commutate_EC_Def* comm_EC, TIM_TypeDef* TIMx)
{
  	...
  	// zero crossing detection measurements emf when the power switch ON
	//Emf_back = (uint16_t)(((long)Uback * (long)56426) >> 16); // check and tune Ktr BackEMF and Ktr Uin
        Emf_back = Uback; // ideal case Ktr BackEMF = Ktr Uin
