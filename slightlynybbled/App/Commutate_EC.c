...
void Init_Control_EC(TIM_TypeDef* TIMx, uint16_t Period, uint16_t CCR_Val, uint16_t DeadTime)
{
	...
	// Channel 1
	TIM_OCInitStructure.TIM_Pulse = Q1.Duty;
	TIM_OC1Init(TIMx, &TIM_OCInitStructure);
	// Channel 2
	TIM_OCInitStructure.TIM_Pulse = Q3.Duty;
	TIM_OC2Init(TIMx, &TIM_OCInitStructure);
	// Channel 3
	TIM_OCInitStructure.TIM_Pulse = Q5.Duty;
...
void Commutate_phase_EC(TIM_TypeDef* TIMx, Commutate_EC_Def* comm_EC)
{
  	...
  	if (comm_EC->Event == FORCE_TR)
	{
	  ...
	  {
                LED_STATUS_ON();                // indication zero crossing 
		Commutate_Pos(TIMx, comm_EC);
		comm_EC->Flag_enable_ZC = 1;	// permit the detection of zero crossing
